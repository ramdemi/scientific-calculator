...
type ScientificConstantDialogProps = {
  isOpen: boolean;
  onDataReceived: (value: string) => void;
};

export const ScientificConstantsDialog = forwardRef(
  function ScientificConstantDialog(
    { isOpen, onDataReceived }: ScientificConstantDialogProps,
    ref,
  ) {
    ...
    // Sync open/close with parent prop
    useEffect(() => {
    ...
    var constants = [
      ...
      },
      {
        ...
        unit: {
          __html:
            "Jmol<sup><small>-1</small></sup>K<sup><small>-1</small></sup>",
        },
      },
      {
    ...
    // When user clicks a constant
    const handleSelect = (value: number) => {
      onDataReceived(String(value)); // send value back to parent
              ...
              <ul>
                {constants.map((c, i) => (
                  ...
                  >
                    <div className="list-item-index">{i + 1}</div>
                    <div>
                      <div className="list-item-display">{c.name}</div>
                      <div className="list-item-description">
                        {c.value} <span dangerouslySetInnerHTML={c.unit} />
                      </div>
                    </div>
                  </li>
                ))}
